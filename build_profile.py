...
def build_skills_json(present: set[str], taxonomy: dict) -> dict:
    ...
    try:
        from ontology import aliases_for as _aliases_for
    except Exception:
        def _aliases_for(labels):  # type: ignore[misc]
            return {lbl: [] for lbl in labels}

    esco_only = sorted(present - set(taxonomy.keys()))
    esco_alias_map = _aliases_for(esco_only)
    for label in esco_only:
        aliases = sorted(set([label.lower()]) | set(esco_alias_map.get(label, [])))
        supported[label] = {
            "aliases": aliases,
...
def _print_summary(present: set[str], taxonomy: dict, out_path: Path) -> None:
    try:
        from ontology import group_for as _group_for_summary
    except Exception:
        def _group_for_summary(lbl: str) -> str:  # type: ignore[misc]
            return "General"
